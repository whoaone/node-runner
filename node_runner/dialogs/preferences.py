...
def load_preferences() -> dict:
    ...
    # v5.3.2 item 69: scalar bar orientation (Results display tab).
    results_bar_orientation = qs.value(
        "results/bar_orientation", "right", type=str)
    if results_bar_orientation not in ('right', 'bottom'):
        results_bar_orientation = 'right'
    try:
    ...
    return {
        ...
        'results_bar_orientation': results_bar_orientation,
        'mystran': mystran,
    }
...
def save_preferences(payload: dict):
    ...
    if 'results_bar_orientation' in payload:
        qs.setValue("results/bar_orientation",
                    str(payload['results_bar_orientation']))
...
class PreferencesDialog(QDialog):
    ...
    def __init__(self, current: dict, parent=None):
        ...
        # v5.3.2 item 69: Results > Bar Position preference.
        self._results_bar_orient_combo: Optional[QComboBox] = None
        ...
        # v5.3.2 item 69: dedicated Results tab for post-processing
        # display preferences (scalar bar position etc.).
    ...
    def _build_results_tab(self):
        """v5.3.2 item 69: Results display preferences.

        Currently houses just the scalar-bar position picker; future
        results-related preferences (default contour style, default
        deform scale, colormap palette) will land here too.
        """
        w = QWidget()
        lay = QFormLayout(w)
        lay.setSpacing(6)
        ...
        section = QLabel("Scalar bar (gradient legend)")
        section.setStyleSheet(
            "font-weight: bold; color: #94e2d5; margin-top: 2px;")
        lay.addRow(section)

        cur_orient = self._initial.get('results_bar_orientation', 'right')
        if cur_orient not in ('right', 'bottom'):
            cur_orient = 'right'
        self._results_bar_orient_combo = QComboBox()
        self._results_bar_orient_combo.addItem("Right (vertical)", "right")
        self._results_bar_orient_combo.addItem("Bottom (horizontal)", "bottom")
        # Set current via data lookup so the persisted key matches.
        for i in range(self._results_bar_orient_combo.count()):
            if self._results_bar_orient_combo.itemData(i) == cur_orient:
                self._results_bar_orient_combo.setCurrentIndex(i)
                break
        self._results_bar_orient_combo.setMinimumWidth(180)
        lay.addRow(QLabel("Bar position:"), self._results_bar_orient_combo)

        note = QLabel(
            "Right = vertical legend on the right edge of the viewport.\n"
            "Bottom = horizontal legend across the bottom.\n"
            "Takes effect on the next contour render."
        )
        note.setWordWrap(True)
        note.setStyleSheet(
            "color: #888; font-style: italic; margin-top: 8px;")
        lay.addRow(note)
        return w
    ...
    def _build_mystran_tab(self):
        ...
        Tooltips throughout assume the user knows MSC Nastran
    ...
    def result_payload(self) -> dict:
        ...
        # v5.3.2 item 69: Results bar orientation.
        results_bar_orientation = 'right'
        if self._results_bar_orient_combo is not None:
            data = self._results_bar_orient_combo.currentData()
            if data in ('right', 'bottom'):
                results_bar_orientation = data
        return {
            'colors': colors,
            'highlight_color': highlight,
            'sizes': sizes,
